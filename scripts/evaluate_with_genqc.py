...
def load_pipeline(model_dir: Path | None, repo_id: str | None, device: torch.device, auth_token: str | None):
    if repo_id:
...
def main():
    ...
    parser.add_argument("--dataset", "-d", required=True, help="Path to unitary-conditioned dataset root.")
    parser.add_argument("--model-dir", help="Local pipeline directory (e.g., scripts/models/my_pipeline).")
    parser.add_argument("--hf-repo", help="Hugging Face repo id to pull a pretrained pipeline from.")
    parser.add_argument("--hf-token", help="Optional Hugging Face auth token.")
    parser.add_argument("--num-samples", type=int, default=64, help="Number of conditions to evaluate.")
    parser.add_argument("--guidance-scale", type=float, default=10.0, help="Guidance scale passed to sampling.")
    parser.add_argument("--sample-steps", type=int, default=20, help="Number of diffusion steps during sampling.")
    parser.add_argument("--auto-batch-size", type=int, default=512, help="Batch size for genQC sampling helper.")
    parser.add_argument("--prompt", default="quantum circuit compilation", help="Text prompt used during sampling.")
    parser.add_argument("--device", choices=["cpu", "cuda"], help="Device to run evaluation on.")
    args = parser.parse_args()

    device = torch.device(args.device or infer_torch_device())
    dataset = load_dataset(Path(args.dataset), device=device)
    ...
    pipeline.scheduler.set_timesteps(args.sample_steps)

    samples = min(args.num_samples, dataset.x.shape[0])
    if samples == 0:
