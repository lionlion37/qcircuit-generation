...
class DatasetGenerator:
    ...
    def __init__(self, config_path: Optional[str] = None, device: Optional[str] = None):
        ...
        self.device = (
            ...
            if "infer_torch_device" in globals()
            else "cpu"
        )
    ...
    @staticmethod
    def _normalize_condition_types(
        condition_type: Union[
            str,
            Iterable[str],
            CircuitConditionType,
            Iterable[CircuitConditionType],
            None,
        ],
    ) -> List[CircuitConditionType]:
        ...
        **kwargs,
        ...
        try:
            ...
            for condition in target_conditions:
                ...
                condition_output = (
                    output_root / condition_name.lower()
                    if multi_condition
                    else output_root
                )
                ...
                dataset_params = {
                    ...
                    else {"x": "tensor", "y": "numpy", "U": "tensor"},
                }
                ...
                mixed_dataset.save_dataset(
                    save_path=str(dataset_path), config_path=str(config_path)
                )

                self.logger.info(
                    f"{condition_name} dataset saved to {condition_output}"
                )
...
class DatasetLoader:
    ...
    def _load_embedder(self, dataset):
        self.vocabulary = {
            gate: idx
            for gate, idx in zip(dataset.gate_pool, range(len(dataset.gate_pool)))
        }
        ...
        if not target:
            module_path = text_encoder_config.pop(
                "module", "my_genQC.models.frozen_open_clip"
            )
            encoder_type = text_encoder_config.get("type")
            if not encoder_type:
                raise ValueError(
                    "Text encoder config requires 'type' or explicit 'target'."
                )
        ...
        cloob_weights_path = self.config["text_encoder"]["params"].get(
            "cloob_weights_path"
        )
        if cloob_weights_path:
            ...
            self.logger.warning(
                f"{len(incomp.missing_keys)} missing keys, first 10: {incomp.missing_keys[:10]}"
            )
            self.logger.warning(
                f"{len(incomp.unexpected_keys)} unexpected keys, first 10: {incomp.unexpected_keys[:10]}"
            )

        return text_encoder

    def _load_single_dataset(
        self, dataset_path: str, load_embedder: bool = True, **kwargs
    ):
    ...
    def load_dataset(self, dataset_path: str, load_embedder: bool = True, **kwargs):
        if "dataset" in os.listdir(dataset_path):
            ...
            if self._get_bucket_batch_size() > 0 and type(dataset) == circuits_dataset.CircuitsConfigDataset:
                order = torch.argsort(dataset.z[:, 0], stable=True)
                dataset.x = dataset.x[order]
                dataset.y = dataset.y[order]
                dataset.z = dataset.z[order]
            ...
            for dataset in os.listdir(parent_dir):
                dataset = self._load_single_dataset(
                    os.path.join(parent_dir, dataset), load_embedder
                )
    ...
    def get_dataloaders(
        self, dataset, batch_size: int = 32, split_ratio: float = 0.1, **kwargs
    ):
