...
class WandbLoggingCallback(Callback):
    ...
    def after_epoch(self, pipeline):
        ...
        if hasattr(pipeline, "_train_epoch_stats"):
            metrics.update(
                {k: float(v) for k, v in pipeline._train_epoch_stats.items()}
            )
...
class DiffusionTrainer:
    ...
    def __init__(
        self, config: Dict[str, Dict[str, Any]] = None, device: Optional[str] = None
    ):
    ...
    def setup_model(self, dataset, text_encoder, tokenizer: Optional = None) -> None:
        ...
        try:
            ...
            if tokenizer is None:
                ...
                gate_pool = getattr(
                    dataset, "gate_pool", ["h", "cx", "cz", "s", "x", "y", "z"]
                )
                vocabulary = {
                    gate: idx for gate, idx in zip(gate_pool, range(len(gate_pool)))
                }
            ...
            model_params["num_clrs"] = (
                len(tokenizer.vocabulary) + 2
            )  # +1 for background, +1 for padding
            ...
            if "unitary_encoder_config" in model_params:
                ...
                if isinstance(unitary_config, dict):
                    model_params["unitary_encoder_config"] = Unitary_encoder_config(
                        **unitary_config
                    )
            ...
            if self.uses_unitary_conditioning:
                self.model = QC_Compilation_UNet.from_config(
                    model_config, self.device, freeze=False
                )
            else:
                self.model = QC_Cond_UNet.from_config(
                    model_config, self.device, freeze=False
                )
            ...
            scheduler_config["target"] = (
                f"genQC.scheduler.scheduler_ddim.{scheduler_config['type']}"
            )
            ...
            if self.uses_unitary_conditioning:
                embedder = QC_Compilation_UNet.from_config(
                    model_config, self.device, freeze=True
                )
            else:
                embedder = QC_Cond_UNet.from_config(
                    model_config, self.device, freeze=True
                )
            ...
            self.pipeline = pipeline_cls(
                ...
                embedder=embedder,
                device=self.device,
                enable_guidance_train=training_config.get(
    ...
    def _setup_wandb(self):
        ...
        try:
            ...
            self.logger.warning(
                "wandb logging requested but the package is not installed."
            )
            return None
    ...
    def train(self, dataloaders, save_path: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            if self.wandb_run:
                cbs = (
                    list(self.pipeline.cbs)
                    if getattr(self.pipeline, "cbs", None)
                    else []
                )
    ...
    def save_model(self, save_path: str) -> None:
        ...
        try:
            ...
            self.pipeline.store_pipeline(
                config_path=save_path + "/", save_path=save_path + "/"
            )
